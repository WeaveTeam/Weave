...
package org.openscales.proj4as.proj {
	...
	public class ProjVandg extends AbstractProjProjection {
		...
		override public function forward(p:ProjPoint):ProjPoint {
			...
			// adufilie: dlon2 is set to EPSLN instead of 0 for al calculation to avoid boundary condition when dlon==0
			var dlon2:Number = Math.abs(dlon) < ProjConstants.EPSLN ? ProjConstants.EPSLN : dlon;
			var al:Number=.5 * Math.abs((ProjConstants.PI / dlon2) - (dlon2 / ProjConstants.PI));
			...
			// adufilie: moved these if statements down here and modified them
			if (Math.abs(lat) <= ProjConstants.EPSLN) {
				x=this.x0 + this.R * dlon;
				y=this.y0;
			}
			else if (Math.abs(Math.abs(lat) - ProjConstants.HALF_PI) <= ProjConstants.EPSLN) {
				if (lat >= 0) {
