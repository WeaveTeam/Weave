...
{	
	...
	system.time(
		...
		for(i in 1:length(l.state))#one loop for every state
		{
			...
			prev_pct<- round(as.data.frame(svymean(myindicator, na.rm=TRUE, design = dstrat))$mean*100,1)
			
            cint<- round((1.96*as.data.frame(svymean(myindicator, na.rm=TRUE, design = dstrat))$SE*100), 1)
			...
			if(i==1)
			 {
				prev.data<-data.frame(l.state[i], sort(unique(myindicator)), prev_pct, conf_int_low, conf_int_hi)
				names(prev.data)<-c("fips", "response", "prev.percent", "CI_LOW", "CI_HI")
			 }
		  else 
			 {
				t.prev.data<-data.frame(l.state[i], sort(unique(myindicator)), prev_pct, conf_int_low, conf_int_hi)
