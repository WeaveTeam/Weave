	...
	var directiveDefnObj= {
			...
			controller : function($scope, $compile, pearsonCoeff,spearmanCoeff, queryService, statisticsService){
				...
				$scope.calculateCoefficient = function(scriptName){
					...
					statisticsService.calculateStats(scriptName, queryService.cache.numericalColumns, true);
			...
			link: function(scope, elem, attrs){
				...
				var drawCorrelationHeatMap = function(data){
					...
					var colorScale = d3.scale.linear()
					     .domain([0, 0.5, 1])//TODO parameterize this according to the matrix  
				...
				scope.$watch(function(){
					return scope.statisticsService.cache.correlationMatrix;
				}, function(newVal, oldVal){
					if(scope.correlationMatrix){
						if(!newVal)
							return;
						else{
							data = statisticsService.cache.correlationMatrix;
							console.log("data", data);
							if(data.length > 0)
							drawCorrelationHeatMap(data[0]);//TODO fix this
						}
					}
				});
