...
package weave
{
	...
	public class VisApplication extends VBox implements ILinkableObject
	{
		...
		private function downloadConfigFile():void
		{
			...
			else if (isLoadedInSwfLoader)
			{
				getCallbackCollection(WeaveAPI.ErrorManager).removeCallback(ErrorLogPanel.openErrorLog);
			}
			else
			{
		...
		private function get isLoadedInSwfLoader():Boolean
		{
			return _flashVars['report'] == 'true';
		}
		...
		private function toggleMenuBar():void
		{
			...
			if ((Weave.properties.enableMenuBar.value || adminService || getFlashVarEditable()) && !isLoadedInSwfLoader)
			{
				if (!_weaveMenu)
				{
...
//						subset.includeMissingKeys.value = true;
		 ...
		 */
		public function handleConfigFileDownloaded(event:ResultEvent = null, token:Object = null):void
		{
			...
			if (getFlashVarEditable() && !isLoadedInSwfLoader)
			{
