...
package weavejs.compiler
{
	...
	import weavejs.compiler.CompiledConstant;
	...
	public class Compiler
	{
		public function Compiler(includeDefaultLibraries:Boolean = true)
		{
			...
			if (includeDefaultLibraries)
				includeLibraries(Math, StandardLib);
		}
		...
		public static function getDefinition(name:String):Object
		{
			...
			return Weave.getDefinition(name);
		}
		...
		public function includeLibraries(...classesOrObjects):void
		{
			for each (var library:Object in classesOrObjects)
			{
				...
				if (library != null && libraries.indexOf(library) < 0)
				{
					...
					else if (Weave.isClass(library))
					{
						className = Weave.className(library as Class);
					}
					if (className)
					{
		...
		private function initialize():void
		{
			...
			// global classes
			for each (var _class:Class in [Array, Boolean, Class, Date, Error, Function, int, Namespace, Number, Object, String, uint])
				globals[Weave.className(_class)] = _class;
			// global functions
			for each (var _funcName:String in 'decodeURI,decodeURIComponent,encodeURI,encodeURIComponent,escape,isFinite,isNaN,isXMLName,parseFloat,parseInt,trace,unescape'.split(','))
				globals[_funcName] = Weave.global[_funcName];
			...
			operators["{"] = function(...args):Object // object creation
			{
				var o:Object = {};
			...
			operators[':'] = function(key:String, value:*):* { this[key] = value; };
			...
			BRANCH_LOOKUP.set(operators[ST_IF], true);
			BRANCH_LOOKUP.set(operators[ST_DO], true);
			BRANCH_LOOKUP.set(operators[ST_WHILE], true);
			BRANCH_LOOKUP.set(operators[ST_FOR], true);
			BRANCH_LOOKUP.set(operators[ST_FOR_DO], true);
			BRANCH_LOOKUP.set(operators['?:'], true);
			BRANCH_LOOKUP.set(operators['&&'], true);
			BRANCH_LOOKUP.set(operators['||'], false);
			
			LOOP_LOOKUP.set(operators[ST_DO], true);
			LOOP_LOOKUP.set(operators[ST_WHILE], true);
			LOOP_LOOKUP.set(operators[ST_FOR], ST_BREAK); // break target only
			LOOP_LOOKUP.set(operators[ST_FOR_DO], ST_CONTINUE); // continue target only
			LOOP_LOOKUP.set(operators[ST_FOR_IN], true);
			LOOP_LOOKUP.set(operators[ST_FOR_EACH], true);
			
			JUMP_LOOKUP.set(operators[ST_BREAK], true);
			JUMP_LOOKUP.set(operators[ST_CONTINUE], true);
			JUMP_LOOKUP.set(operators[ST_RETURN], true);
			JUMP_LOOKUP.set(operators[ST_THROW], true);

			var func:Function;

			for each (func in pureOperators)
				PURE_OP_LOOKUP.set(func, true);
				
			for each (func in assignmentOperators)
				ASSIGN_OP_LOOKUP.set(func, true);
		}
		...
		private function compileTokens(tokens:Array, allowSemicolons:Boolean):ICompiledObject
		{
			...
			{
				token = tokens[i];
				if (token == OPERATOR_ESCAPE && typeof operators[tokens[i + 1]] === 'function')
				{
					token = tokens[i + 1];
			...
			conditionals: while (true)
			{
				...
				{
					if (typeof tokens[end] === 'string' && '?:,;'.indexOf(tokens[end]) >= 0)
						break;
					end++;
				}
				if (debug)
		...
		*/
			...
			while (true)
			{
				...
				if (leftBracket == '{')
				{
					...
					if (blockCall)
					{
						...
						if (blockItems && (blockItems.length == 0 || StandardLib.getArrayType(blockItems) == CompiledFunctionCall))
						{
							var params:Array = [];
							for each (var item:CompiledFunctionCall in blockItems)
							{
								if (item.evaluatedMethod != operators[':'])
								{
									params = null;
									break;
								}
							...
							if (params != null)
							{
								tokens.splice(open, 2, compileOperator('{', params));
								continue;
							}
						}
					}
					...
					tokens.splice(open, 2, compileOperator(';', [block]));
					continue;
				}
				...
				if (leftBracket == '(' && compiledParams.length == 0 && (token != FUNCTION && tokens[open + 2] != '=>'))
		...
		private function compileUnaryOperators(compiledTokens:Array, operatorSymbols:Array):void
		{
			...
			while (index--) // right to left
			{
				...
				if ((nextToken === undefined && token != '#') || typeof nextToken === 'string')
		...
		private function compileInfixOperators(compiledTokens:Array, operatorSymbols:Array):void
		{
			...
			while (index < compiledTokens.length)
			{
				...
				while (right < compiledTokens.length && typeof compiledTokens[right] === 'string')
					right++;
				...
				if (index == 0 || index + 1 == compiledTokens.length || typeof compiledTokens[index - 1] === 'string' || typeof compiledTokens[index + 1] === 'string')
			...
			nextPattern: for each (var pattern:Array in _validStatementPatterns)
			{
				for (var iPattern:int = 0; iPattern < pattern.length; iPattern++)
				{
					...
					if (type == PN_VARS)
					{
						...
						if (isVariableLookup(call) || call.evaluatedMethod == operators['='])
		...
		public function compileObjectToFunction(compiledObject:ICompiledObject, symbolTable:Object, errorHandler:Function, useThisScope:Boolean, paramNames:Array = null, paramDefaults:Array = null, flattenFunctionDefinition:Boolean = true, bindThis:Object = null):Function
		{
			...
			var allSymbolTables:Array = [builtInSymbolTable]; // buit-in first
			...
			var THIS_SYMBOL_TABLE_INDEX:int = allSymbolTables.push(null) - 1;
					...
					try
					{
						...
						else if (method == operators[ST_IMPORT])
						{
							for each (result in call.evaluatedParams)
							{
								...
								if (!symbolName)
									symbolName = Weave.className(Weave.asClass(result));
						...
						else if (Weave.isClass(method))
						{
							...
							else if (method === Class && typeof call.evaluatedParams[0] === 'string')
							{
								result = getDefinition(call.evaluatedParams[0]);
							}
							else // all other single-parameter type casting operations
							{
								result = cast(call.evaluatedParams[0], Weave.asClass(method));
							}
						}
						...
						else if (method == operators[FUNCTION] || method == operators['=>']) // inline function definition
						{
						...
						else if (call.evaluatedHost is ProxyObject)
						{
							...
							result = (call.evaluatedHost as ProxyObject).callProperty.apply(call.evaluatedHost, proxyParams);
						}
						else
						{
