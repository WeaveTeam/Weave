...
package weave.visualization.plotters
{
	...
	public class GraphPlotter extends AbstractPlotter
	{
		public function GraphPlotter()
		{
			...
			layoutAlgorithm.setInternalObject(ForceDirectedLayout);
		...
		public function init():void
		{
			...
			layoutAlgorithm.internalObject.initRService(Weave.properties.rServiceURL.value);
			//resetAllNodes();
		}
		...
		{
			layoutAlgorithm.internalObject.updateDraggedKeys(keys, dx, dy, runSpatialCallbacks);
			layoutAlgorithm.internalObject.getOutputBounds(null, tempBounds);
		...
		public function scaleNodes(keys:Array, scaleFactor:Number = 2):void
		{
			...
			{
				node = layoutAlgorithm.internalObject.getNodeFromKey(key);
				if (!node)
					continue;
				nodes.push(node);
			...
			layoutAlgorithm.internalObject.updateOutputBounds();
			spatialCallbacks.triggerCallbacks();
			getCallbackCollection(this).triggerCallbacks();
		}
		...
		{
			layoutAlgorithm.internalObject.resetAllNodes();
			_iterations = 0;
			layoutAlgorithm.internalObject.getOutputBounds(null, tempBounds);
		}
//		/**
...
//		}
		...
		public function continueComputation(keys:Array):void
		{
			...
			if (!shouldStop.value)
			{
				layoutAlgorithm.internalObject.getOutputBounds(keys, tempBounds);
				layoutAlgorithm.internalObject.incrementLayout(keys, tempBounds);
			}
		...
		private function changeAlgorithm():void
		{
			...
			layoutAlgorithm.setInternalObject(newAlgorithm);
			layoutAlgorithm.internalObject.initRService(Weave.properties.rServiceURL.value);
			handleColumnsChange();
			layoutAlgorithm.internalObject.setupData(nodesColumn, edgeSourceColumn, edgeTargetColumn);
			spatialCallbacks.triggerCallbacks();
			getCallbackCollection(this).triggerCallbacks();
		}
		...
		public const sizeColumn:DynamicColumn = registerNonSpatialProperty(new DynamicColumn());
		...
		public const labelColumn:DynamicColumn = registerNonSpatialProperty(new DynamicColumn());
		...
		public const layoutAlgorithm:DynamicGraphAlgorithm = newSpatialProperty(DynamicGraphAlgorithm);
		...
		public const drawCurvedLines:LinkableBoolean = registerNonSpatialProperty(new LinkableBoolean(true)); // should we draw curved lines instead of a gradient?
...
//		private var _isDragging:Boolean = false;
		...
		override public function drawPlot(recordKeys:Array, dataBounds:IBounds2D, screenBounds:IBounds2D, destination:BitmapData):void
		{
			...
			var keys:Array = layoutAlgorithm.internalObject.getNeighboringKeys(recordKeys);
			// loop through each node and draw it
			for (var iKey:int = keys.length - 1; iKey >= 0; --iKey)
			{
				key = keys[iKey];
				var node:IGraphNode = layoutAlgorithm.internalObject.getNodeFromKey(key);
				if (!node)
					continue;
		...
		override public function getDataBoundsFromRecordKey(recordKey:IQualifiedKey):Array
		{
			...
			var node:IGraphNode = layoutAlgorithm.internalObject.getNodeFromKey(recordKey);
			var keyPoint:Point;
			var edgePoint:Point;
			if (node)
			{
		...
		override public function getBackgroundDataBounds():IBounds2D
		{
			...
			layoutAlgorithm.internalObject.getOutputBounds(null, b);
