...
package
{
	public class WeavePath
	{
		 ...
		 * @private
		 */
		private static var _tempVars:Object = {};
		
		/**
		 * Cleans up temporary variables.
		 * @private
		 */
		private static function _deleteTempVars():void
		{
		 ...
		 * @private
		 */
		private static function _A(args:Array, option:int = 0):Array
		{
		...
		public function push(...relativePath):WeavePath
		{
			...
			var newWeavePath:WeavePath = new WeavePath(this.weave, this._path.concat(args));
		 ...
		 * @return The current WeavePath object.
		 */
		public function addCallback(callback, triggerCallbackNow, immediateMode, delayWhileBusy):WeavePath
		{
		...
		public function getChildren(...relativePath):Array
		{
			...
			return this.weave.directAPI.getChildNames(this._path.concat(relativePath))
				.map(function(name:String):WeavePath { return new WeavePath(this.weave, this._path.concat(relativePath, name)); }, this);
		}
		 ...
		 */
		public function getValue(script_or_variableName):*
		{
			_vars[''] = Weave.objectKeys(_vars); // include a list of keys in property '' so undefined variables will be preserved
		...
		// helper functions
		private static function _assertParams(methodName:String, args:Array, minLength:int = 1):Boolean
		{
			if (!minLength)
				minLength = 1;
		...
		private static function _failPath(methodName:String, path:Array):void
		{
			_failMessage(methodName, 'command failed', path);
		}
		
		private static function _failObject(methodName:String, path:Array):void
		{
			_failMessage(methodName, 'object does not exist', path);
		}
		
		private static function _failMessage(methodName:String, message:String, path:Array = null):void
		{
