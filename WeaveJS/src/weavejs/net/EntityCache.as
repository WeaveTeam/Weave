...
package weavejs.net
{
    ...
    import weavejs.util.CallbackUtils;
    ...
    public class EntityCache implements ILinkableObject, IDisposableObject
    {
		...
		private function get callbacks():ICallbackCollection { return Weave.getCallbacks(this); }
        ...
        public function EntityCache(service:IWeaveEntityService, purgeMissingEntities:Boolean = false)
        {
			...
			callbacks.addImmediateCallback(this, CallbackUtils.generateDelayedCallback(this, delayedCallback, 0));
        }
		...
		private function delayedCallback():void
		{
			...
			{
				adminService.removeEntities(idsToRemove).then(handleIdsToInvalidate.bind(this, true));
				idsToDelete = {};
			}
			...
			for (id in idsToFetch)
			{
				...
				if (id == ROOT_ID)
				{
					...
					service.getHierarchyInfo(tableMetadata).then(handleEntityHierarchyInfo.bind(this, tableMetadata));
					
					var hierarchyMetadata:Object = {};
					hierarchyMetadata[ColumnMetadata.ENTITY_TYPE] = EntityType.HIERARCHY;
					service.getHierarchyInfo(hierarchyMetadata).then(handleEntityHierarchyInfo.bind(this, hierarchyMetadata));
				}
				else
					ids.push(int(id));
			}
			...
			while (ids.length > 0)
			{
				...
				service.getEntities(splicedIds).then(getEntityHandler.bind(this, splicedIds));
			}
			
			WeaveAPI.SessionManager.unassignBusyTask(delayedCallback);
        }
		
		private function handleIdsToInvalidate(alsoInvalidateRelatives:Boolean, result:Array):void
		{
			callbacks.delayCallbacks();
			
			for each (var id:int in result)
				invalidate(id, alsoInvalidateRelatives);
			
			callbacks.resumeCallbacks();
		}
		
        private function getEntityHandler(requestedIds:Array, result:Array):void
        {
			var id:int;
			var entity:Entity;
		...
		private function handleEntityHierarchyInfo(publicMetadata:Object, result:Array):Array
		{
			...
			var keys:Array = JS.objectKeys(publicMetadata);
...
//			}
		...
		public function update_metadata(id:int, diff:EntityMetadata):void
        {
			if (!adminService)
			{
				JS.error("Unable to update metadata (Not an admin service)");
				return;
			}
        ...
        public function add_category(title:String, parentId:int, index:int):void
        {
			if (!adminService)
			{
				JS.error("Unable to create entities (Not an admin service)");
				return;
			}
        ...
        public function add_child(parent_id:int, child_id:int, index:int):void
        {
			if (!adminService)
			{
				JS.error("Unable to modify hierarchy (Not an admin service)");
				return;
			}
			...
			adminService.addChild(parent_id, child_id, index)
				.then(handleIdsToInvalidate.bind(this, false));
        ...
        public function remove_child(parent_id:int, child_id:int):void
        {
			if (!adminService)
			{
				JS.error("Unable to remove entities (Not an admin service)");
				return;
			}
